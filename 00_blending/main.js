...
var blendOptions; // all blend options
...
loadResources({
  ...
  texture_grass: '../textures/grass.png',
  texture_window: '../textures/blending_transparent_window.png',
...
function createSceneGraph(gl, resources) {
  ...
  {
    //initialize wall
    textureNode = new TextureSGNode(resources.texture_diffuse3, 0, 'u_diffuseTex',
                    new RenderSGNode(makeRect()));
    let wall = new MaterialSGNode( textureNode  );
    textureNode.init(gl);
    toggleAnisotropicFiltering(true);
    toggleMipmapping(true);
    ...
    wall.diffuse = [0.1, 0.5, 0.1, 1];
    wall.specular = [0.5, 0.5, 0.5, 1];
    wall.shininess = 50.0;

    root.append(new TransformationSGNode(glm.transform({ translate: [-1,0,1], rotateZ: 180, scale: 1}), [
      wall
    ]));
  }
  ...
  {
    //initialize grass
    textureNode = new TextureSGNode(resources.texture_grass, 0, 'u_diffuseTex',
                    new RenderSGNode(makeRect()));
    let grass = new MaterialSGNode( textureNode  );
    textureNode.init(gl);
    toggleAnisotropicFiltering(true);
    toggleMipmapping(true);

    //dark
    grass.ambient = [0, 0, 0, 1];
    grass.diffuse = [0.1, 0.5, 0.1, 1];
    grass.specular = [0.5, 0.5, 0.5, 1];
    grass.shininess = 50.0;

    root.append(new TransformationSGNode(glm.transform({ translate: [0,0,0], rotateZ: 180, scale: 1}), [
      grass
    ]));
  }
  ...
  {
    //initialize window
    textureNode = new TextureSGNode(resources.texture_window, 0, 'u_diffuseTex',
                    new RenderSGNode(makeRect()));
    let window = new MaterialSGNode( textureNode  );
    textureNode.init(gl);
    toggleAnisotropicFiltering(true);
    toggleMipmapping(true);
    ...
    window.diffuse = [0.1, 0.5, 0.1, 1];
    window.specular = [0.5, 0.5, 0.5, 1];
    window.shininess = 50.0;
...
var blendFuncSourceFactor;
var blendFuncDstFactor;
...
function initGUI(){
  ...
  */
  ...
  let sourceFactor = function(){}; sourceFactor.source = Object.keys(blendOptions)[6]; // "SRC_ALPHA"; 
  blendFuncSourceFactor = blendOptions[Object.keys(blendOptions)[6]];
  gui.add( sourceFactor, 'source', Object.keys(blendOptions) ).onChange(function(value){
    blendFuncSourceFactor = blendOptions[value];
    gl.blendFunc(blendFuncSourceFactor, blendFuncDstFactor);
  });

  let destFactor = function(){}; destFactor.dest = Object.keys(blendOptions)[7]; //  "ONE_MINUS_SRC_ALPHA"; 
  blendFuncDstFactor = blendOptions[Object.keys(blendOptions)[7]];
  gui.add( destFactor, 'dest', Object.keys(blendOptions) ).onChange(function(value){
    blendFuncDstFactor = blendOptions[value];
    gl.blendFunc(blendFuncSourceFactor, blendFuncDstFactor);
  });
